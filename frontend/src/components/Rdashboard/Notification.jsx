...
//       <button
//         className="cursor-pointer p-2 rounded-full bg-black transition relative"
...
const Notification = ({
  ...
  notificationClickCallback,
  customIcon,
  panelClassName = "text-black" // Default to black text
}) => {
  const navigate = useNavigate();
  const [showNotifications, setShowNotifications] = useState(false);
  const [notifications, setNotifications] = useState(initialNotifications);
  ...
  return (
    <div className="relative" ref={notificationRef}>
      ...
      {showNotifications && (
        <div className={`absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-lg border border-gray-200 z-50 ${panelClassName}`}>
          <div className="p-3 border-b border-gray-200 flex justify-between items-center">
            <h3 className="font-bold">Notifications</h3>
            <div className="flex items-center gap-2">
              <button
