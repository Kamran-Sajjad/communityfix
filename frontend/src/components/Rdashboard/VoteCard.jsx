...
//             {description && (
//               <p
//                 className={`text-sm md:text-base text-gray-600 mt-2 ${!isExpanded && "line-clamp-3"
//                   }`}
//               >
//                 {description}
//               </p>
...
//           onClick={handleVote}
//           disabled={hasVoted || loading} // Disable the button if the user has already voted
//           className={`bg-black text-white px-6 py-2 rounded-full text-sm md:text-base transition-opacity duration-200 ${hasVoted || loading ? "opacity-50 cursor-not-allowed" : ""
//             }`}
//         >
//           {hasVoted ? "Voted" : loading ? "Voting..." : "Vote"}
//         </button>
...
export default function VoteCard({
 ...
 const handleVote = async () => {
  ...
  if (hasVoted) {
    showWarningToast("You have already voted for this issue.");
    return;
  }
