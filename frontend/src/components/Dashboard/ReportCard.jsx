...
const ReportCard = ({ report }) => (
  <div className="w-full bg-gray-200 p-6 rounded-lg shadow-md mb-4 flex flex-wrap md:flex-nowrap justify-between items-start gap-4">
    <div className="flex-1 min-w-[200px]">
      <div className="flex flex-wrap items-center gap-2">
        <h3 className="text-lg font-bold text-gray-700">{report.name}</h3>
      ...
      <p className="text-sm text-gray-600 mt-1">
        {report.age} {report.age === 1 ? "hour" : "hours"} old
      </p>
      <p className="text-sm text-gray-700 mt-2 font-semibold">{report.issueTitle}</p>
      <p className="text-sm text-gray-600 mt-1">{report.issueDescription}</p>
      <div className="mt-4">
    ...
    <div className="flex items-center gap-2 md:gap-4 mt-4 md:mt-0">
      <button className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 transition">X</button>
      <button className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 transition">✓</button>
    </div>
  </div>
);
